...
/*   Updated: 2025/02/12 04:06:43 by uanglade         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	create_tileset2(t_data ts, t_tileset *ret, t_var var)
{
	...
	while (++i < ret->width)
	{
		...
		if (!ret->tileset)
			return (clean_and_exit("Failed to malloc tileset\n", var, 1));
		while (++j < ret->height)
		{
			ret->tileset[i][j].img = mlx_new_image(var.mlx, 16, 16);
			if (!ret->tileset[i][j].img)
				return (clean_and_exit("Failed to load tileset\n", var, 1));
...
t_tileset	create_tileset(char *rel_path, t_var var, int width, int height)
{
	...
	ts.addr = mlx_get_data_addr(ts.img, &ts.bits_per_pixel, &ts.line_length,
			&ts.endian);
	if (!ts.addr)
		return (clean_and_exit("Failed to get img address\n", var, 1), ret);
