...
/*   Updated: 2025/02/12 04:23:09 by uanglade         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_key(int key_code, t_var *var)
{
		...
		|| key_code == KEY_RIGHT)
	{
			...
			key_code))
		{
			...
			ft_putnbr_fd(var->player.move_count, 1);
			write(1, "\n", 1);
		}
	}
...
int	main(int ac, char **av)
{
	...
	if (!is_map_valid(var.map, var.player, var))
		return (clean_and_exit("Map not solvable\n", var, 1), 1);
